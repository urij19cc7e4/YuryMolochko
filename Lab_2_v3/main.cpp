...
class comp_fract
{
	friend ostream& operator<<(ostream& out, const comp_fract& f);
...
public:
	comp_fract(int comp = 0, int numer = 0, int denom = 1)
	{
		if (denom == 0)
		{
			this->comp = 0;
	...
	comp_fract(fract& f) : comp(f.numer / f.denom), numer(f.numer% f.denom), denom(f.denom) {};
};
...
ostream& operator<<(ostream& out, const comp_fract& f)
{
	if (f.comp)
		cout << f.comp << "#";
	out << f.numer << "/" << f.denom;
...
int main()
{
	...
	cout << f1 << " = " << (comp_fract)f1 << "\n";
	cout << f2 << " = " << (comp_fract)f2 << "\n";
	cout << f3 << " = " << (comp_fract)f3 << "\n";
}